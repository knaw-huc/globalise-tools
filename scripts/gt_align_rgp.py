...
def main() -> None:
    ...
    if os.path.exists("gm-alignment.store.stam.cbor"):
        ...
        for inv_nr in sorted(inv_nrs):
            ...
            with open(htr_file, mode='r') as file_inv:
                ...
                htr_lines = [store.annotate(id=line_id, target=stam.Selector.textselector(htr_textsel,
                                                                                          stam.Offset.simple(begin,
                                                                                                             end)),
                                            data=LINE_TYPE_DATA) for line_id, begin, end in id_annotations]

                # associate the page annotations (HTR)
                for (page, begin) in sorted(pagebegin.items()):
                    offset = stam.Offset.simple(begin, pageend[page])
                    store.annotate(id=f"NL-HaNA_1.04.02_{inv_nr}_{page}",
                                   data=[PAGE_TYPE_DATA, {"set": "globalise", "key": "page", "value": page}],
                                   target=htr_textsel.textselection(offset).select())
    ...
    PAGE_TYPE_DATA_INSTANCE = next(
        store.data(PAGE_TYPE_DATA))  # grab the instance to prevent having to re-resolve it every time later
    ...
    for rgp_letter in store.data(LETTER_TYPE_DATA).annotations():
        ...
        try:
            ...
            continue
        rgp_letter_header = rgp_letter_textsel.resource().textselection(
            stam.Offset.simple(rgp_letter_textsel.begin(), newline.begin())).text().strip()
        # strip header from the letter
        rgp_letter_textsel = rgp_letter_textsel.resource().textselection(
            stam.Offset.simple(newline.begin() + 1, rgp_letter_textsel.end()))
        ...
        try:
            ...
            print(
                f"No match in metadata mapping for letter {letter_id} from RGP vol {rgp_vol} page >= {rgp_startpage} -- \"{rgp_letter_header}\"",
                file=sys.stderr)
            continue
        print(
            f"Found match in metadata mapping for letter {letter_id} from RGP vol {rgp_vol} page >= {rgp_startpage} -- \"{rgp_letter_header}\"",
            file=sys.stderr)
        ...
        for rgp_paragraph_textsel in rgp_letter_textsel.related_text(stam.TextSelectionOperator.embeds(),
                                                                     filter=PARAGRAPH_TYPE_DATA):
            print(
                f"Adding paragraph {rgp_paragraph_textsel.offset()} from letter {letter_id} from RGP vol {rgp_vol} page >= {rgp_startpage} with inv_nr {inv_nr} scans {htr_beginpage}-{htr_endpage}...",
                file=sys.stderr)
            ...
            try:
                ...
                continue
            htr_textsel = store.resource(htr_resource_id).textselection(
                stam.Offset.simple(htr_beginpage_ts.begin(), htr_endpage_ts.end()))
    ...
    for translations, (rgp_paragraph_textsel, htr_textsel), (rgp_vol, rgp_startpage, letter_id, htr_resource_id, inv_nr,
                                                             htr_beginpage, htr_endpage) in zip(results, align_pairs,
                                                                                                metadata):
        print(
            f"Aligned paragraph {rgp_paragraph_textsel.offset()} from letter {letter_id} from RGP vol {rgp_vol} page >= {rgp_startpage} with inv_nr {inv_nr} scans {htr_beginpage}-{htr_endpage}...",
            file=sys.stderr)
        print(f"   computed {len(translations)} translation(s)", file=sys.stderr)
        if args.verbose:
            print(f"<<<<<<< RGP {rgp_vol} {rgp_startpage} {letter_id} {rgp_paragraph_textsel.offset()}",
                  file=sys.stderr)
        ...
        for translation in translations:
            ...
            for alignment in translation.alignments():
                ...
                try:
                    htr_page_ts = next(htr_paragraph.related_text(stam.TextSelectionOperator.overlaps(),
                                                                  filter=PAGE_TYPE_DATA_INSTANCE))
                ...
                rgp_paragraph_text = str(rgp_paragraph_textsel).replace("\n", "\\n")
                htr_paragraph_text = str(htr_paragraph).replace("\n", "\\n")
                print(
                    f"{rgp_vol}\t{rgp_startpage}\t{letter_id}\t{rgp_paragraph_textsel.offset()}\t{htr_resource_id}\t{htr_page}\t{htr_paragraph.offset()}\t{rgp_paragraph_text}\t{htr_paragraph_text}")
        if args.verbose:
