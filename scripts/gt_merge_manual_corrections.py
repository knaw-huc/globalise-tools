...
def main():
    ...
    parser.add_argument("--all", help="Add corrected entries even if they were not found in the original data",
                        action="store_true")
            ...
            for row in reader:
                ...
                if filename == args.inputfile:
                    ...
                    if key not in data and not args.all:
                        print(f"Warning: key from corrections not found in original data {args.inputfile}: ", key,
                              file=sys.stderr)
                        continue
                data[key] = row
    ...
    writer = csv.DictWriter(sys.stdout, fieldnames=fieldnames, delimiter="\t", quoting=csv.QUOTE_NONE,
                            extrasaction='ignore', escapechar="\\")
    writer.writeheader()
