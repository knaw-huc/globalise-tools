...
process-inventory() {
  local invnrs=("$@")
  make --jobs 3 --keep-going $(for i in "${invnrs[@]}"; do printf "upload-csv-$i "; done)
  for invnr in "${invnrs[@]}"; do
    echo $invnr >> work/csv-inv-done.lst
  done
}
...
main() {
  ...
  local len=${#invnrs[@]}
  echo "uploading $len entity-linking csvs"
  ...
  local run=$(seconds-to-dhms $elapsed_time)
  echo "done in $run"
  send-notification "gt-upload-all-entity-linking-csv finished in $run"
}
