...
process-inventory() {
  local invnrs=("$@")
  make --jobs 3 --keep-going $(for i in "${invnrs[@]}"; do printf "upload-index-$i "; done)
  for invnr in "${invnrs[@]}"; do
    echo $invnr >> work/index-inv-done.lst
  done
}
...
main() {
  ...
  readarray -t invnrs < work/inventorynumbers.lst
  local len=${#invnrs[@]}
  echo "uploading $len inventory indices"
  ...
  local run=$(seconds-to-dhms $elapsed_time)
  echo "done in $run"
  send-notification "gt-upload-all-inventory-indices finished in $run"
}
