...
process-inventory() {
  local invnrs=("$@")
  make --jobs 3 --keep-going $(for i in "${invnrs[@]}"; do printf "work/$i/index.json "; done)
...
main() {
  ...
  local len=${#invnrs[@]}
  echo "making $len inventory indices"
  ...
  local run=$(seconds-to-dhms $elapsed_time)
  echo "done in $run"
  send-notification "gt-make-all-inventory-indices finished in $run"
}
